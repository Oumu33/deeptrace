...
| --- | --- | --- |
| `count` | `3500` | 当前状态的连接数 |
| `established` | `12000` | ESTABLISHED 连接数（提供背景参考） |

`established` 在 Netlink 路径下零额外成本（遍历消息时顺便计数），帮助用户判断异常连接的相对比例——"3500 CLOSE_WAIT + 12000 ESTABLISHED"比单独看 3500 更有信息量。

仅在 Netlink 路径（配了 close_wait）时携带 `established`；sockstat 快速路径不携带（sockstat 文件中无此信息）。
...
emitStateEvent(q, check, count, config, establishedCount):
    ...
    attrs = map[string]string{"count": strconv.FormatUint(count, 10)}
    if establishedCount >= 0:
        attrs["established"] = strconv.FormatUint(establishedCount, 10)
