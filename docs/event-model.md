...
deeptrace 的核心产出是**事件（Event）**。每个插件在每次采集时，根据检查结果产出一个或多个事件，由 engine 处理后推送到 FlashDuty。
...
1. FlashDuty 可以通过 labels 和 attrs 获取所有结构化数据，在不同通知渠道（短信、邮件、IM）灵活渲染
...
告警标题由 FlashDuty 输出层根据事件自动生成：若事件有 `target` 标签则使用 `[TPL]${check} ${from_hostip} ${target}`，否则使用 `[TPL]${check} ${from_hostip}`。插件无需配置 title_rule。
