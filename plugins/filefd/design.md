...
**参考**：Prometheus `node_exporter` 的 filefd collector。node_exporter 采集 `node_filefd_allocated` 和 `node_filefd_maximum` 指标，deeptrace 直接计算使用率并在逼近上限时主动告警。
...
在容器内读取 `/proc/sys/fs/file-nr` 获取的是**宿主机**的值（默认 namespace 下 proc 不隔离）。若 deeptrace 运行在容器内，监控的是宿主机的文件描述符使用率。这通常是期望的行为——系统级 fd 是宿主机级别资源，在容器内监控正好覆盖"宿主机 fd 耗尽导致容器无法创建连接"的场景。
