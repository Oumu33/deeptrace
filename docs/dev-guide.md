...
本文档帮助新开发者（以及 AI 助手）快速掌握 deeptrace 的全貌，避免从零探索带来的时间和 token 浪费。

## 项目定位

deeptrace 是一个**轻量级智能主机监控 Agent**，定位与 Nagios/Sensu 类似但更现代：
...
2. **AI 排障**：包括自动诊断（告警触发）和交互 Chat（人工登录机器后用自然语言排查）。运维人员不用记忆复杂命令，`deeptrace chat` 即可调用 70+ 诊断工具和 shell 命令来定位问题
...
7. AI 多轮对话：调用诊断工具/MCP 工具 → 生成报告 → `forwardReport()` 为每个 AlertKey 创建新事件推到 notify
...
通过 stdio JSON-RPC 2.0 与外部 MCP Server 通信（如 Prometheus、Jaeger）：

- `Manager.StartAll()` 启动所有配置的 MCP Server 进程
- 工具注册到 `ToolRegistry`，类别前缀 `mcp:`，工具名前缀 `{server}_`
- 白名单过滤：`tools_allow` 配置
- 身份标识：`identity` 字段，格式如 `ident="my-host"`，告诉 AI 如何过滤本机数据
- 诊断提示词对 MCP 工具采用"摘要模式"（名称+描述+数量），避免 Token 浪费
...
| 命令 | 说明 |
| ------ | ------ |
| `deeptrace run` | 启动 Agent（`--interval`、`--plugins` 过滤） |
| `deeptrace chat` | 交互式 AI 对话（`-v` 详细、`--model` 指定模型） |
| `deeptrace inspect <plugin> [target]` | 主动 AI 健康检查 |
| `deeptrace diagnose list` | 列出诊断记录 |
| `deeptrace diagnose show <id>` | 查看诊断详情 |
| `deeptrace selftest [filter]` | 诊断工具冒烟测试（`-q` 安静模式） |
| `deeptrace mcptest` | 测试 MCP 连接 |
...
| 新增 MCP 协议能力 | `mcp/client.go` |
| 修改事件处理逻辑 | `engine/engine.go` |
| 修改配置结构 | `config/config.go` + `config/inline.go` + `config/mcp_config.go` |
