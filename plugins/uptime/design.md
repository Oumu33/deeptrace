...
**参考**：Nagios `check_uptime`。Nagios 的 check_uptime 默认检查 uptime 是否**超过**阈值（用于检测需要重启打补丁的机器），deeptrace 取反——检查 uptime 是否**低于**阈值（用于检测异常重启），更符合 On-call 场景的需要。
...
- `host.BootTime()` — 返回开机 Unix 时间戳（`uint64`），用于 attrs 的 `boot_time`
...
| 属性 | 示例值 | 说明 |
| --- | --- | --- |
| `uptime` | `2d 5h 30m` | 人类可读的 uptime |
| `uptime_seconds` | `191400` | 原始秒数 |
| `boot_time` | `2026-02-24 08:30:00 CST` | 系统启动时间（本地时区，附带时区标识） |
| `critical_lt` | `10m` | 配置的 critical 阈值（humanDuration 格式，与 uptime 风格一致） |
| `warn_lt` | `1h` | 配置的 warn 阈值（仅配置时显示） |

OK 事件也携带完整 attrs，便于巡检时确认系统运行时长和启动时间。
...
Gather(q):
    ...
    bootTimeSec, _ = host.BootTime()   // non-fatal: 仅用于 attrs
    ...
    attrs = map[string]string{
        "uptime": humanDuration(uptimeDur),
        "uptime_seconds": strconv.FormatUint(uptimeSec, 10),
        "critical_lt": humanDuration(time.Duration(ins.RebootDetected.CriticalLt)),
    }
    if bootTimeSec > 0:
        attrs["boot_time"] = time.Unix(int64(bootTimeSec), 0).Format("2006-01-02 15:04:05 MST")
    if ins.RebootDetected.WarnLt > 0:
        attrs["warn_lt"] = humanDuration(time.Duration(ins.RebootDetected.WarnLt))
...
3. **`host.BootTime()` 失败仅影响 attrs 的 `boot_time` 不显示**，不阻塞主逻辑（原则 13：局部失败不影响全局）
...
`host.Uptime()` 在容器内读取的是**宿主机**的 uptime（Linux `/proc/uptime` 在默认 namespace 下不隔离），而非容器自身的运行时间。若 deeptrace 运行在容器内，检测的是宿主机重启，非容器重启。容器重启检测由 docker 插件覆盖。
