...
deeptrace 当前的工作流是：**探测问题 → 推送 FlashDuty → 用户手动排查**。用户收到告警后，需要登录机器、执行各种命令才能定位根因，这个过程既慢又依赖经验。

AI 辅助诊断的目标：**告警触发时自动诊断，将问题和根因分析一起推送给用户**，结构性缩短 MTTR。

```
当前：deeptrace → FlashDuty → 人登录机器排查 → 开始修复
目标：deeptrace → 自动诊断 → FlashDuty 推送「问题 + 根因 + 建议」→ 人直接修复
```
...
deeptrace 充当 AI agent 的 tool executor：AI 决定执行哪些诊断命令，deeptrace 执行后返回结果，AI 据此推理下一步或输出最终报告。
...
deeptrace 不把具体工具注册为 AI 的 function，而是提供三个元工具让 AI 按需发现：
...
**关键 metrics**（通过 deeptrace 现有 metrics 机制暴露）：
...
deeptrace 不给 AI 通用 shell 执行能力，而是让每个插件定义受限的、只读的诊断操作。
...
**存储位置**：`state.d/diagnoses/`，与 deeptrace 现有目录规范一致（`conf.d` 放配置、`state.d` 放运行时状态数据）。
...
deeptrace diagnose list --target=10.0.0.1:6379
deeptrace diagnose list --check=redis::used_memory
deeptrace diagnose list --since=1h

# 查看某条诊断的完整详情
deeptrace diagnose show d8f3a2b1

# 查看某条诊断的某一轮工具输出
deeptrace diagnose show d8f3a2b1 --round=2
```
...
deeptrace 启动时和每次写入新记录后检查，超期或超量的旧记录自动删除。

**与 FlashDuty 的联动**：FlashDuty description 中包含诊断 ID，用户可据此查看完整记录：

```
详情: deeptrace diagnose show d8f3a2b1
```
...
截断后在末尾标注 `[已截断] 详情: deeptrace diagnose show <id>`，完整报告存储在本地诊断记录中。
...
⚠️ AI 辅助诊断，仅供参考 | 详情: deeptrace diagnose show d8f3a2b1
```
...
func (e *DiagnoseEngine) RunDiagnose(req *DiagnoseRequest) {
    ...
    // panic recovery：符合 deeptrace principles.md 要求
    defer func() {
        if r := recover(); r != nil {
...
```
你是一位资深运维和 DBA 专家。deeptrace 监控系统检测到以下告警：
...
{{if .IsRemoteTarget}}- ⚠️ 目标 {{.Target}} 是远端主机，本机基础设施工具（disk、cpu、memory 等）
  反映的是 deeptrace 所在主机 {{.LocalHost}} 的状态，不是目标主机的状态
  这些工具的结果仅在 deeptrace 与目标部署在同一台机器时有参考价值
{{else}}- deeptrace 与目标 {{.Target}} 在同一台机器上，本机基础设施工具可直接用于辅助诊断
{{end}}
## 输出要求
...
首版通过 `deeptrace diagnose list / show` CLI 查询本地诊断记录。后续可预留 HTTP 端点：
...
│   ├── cli.go                       # CLI: deeptrace diagnose list/show（直接在 diagnose 包内）
...
```go
// RunDiagnoseList(args)    deeptrace diagnose list [--target=...] [--since=...]
// RunDiagnoseShow(args)    deeptrace diagnose show <id> [--round=N]
```
