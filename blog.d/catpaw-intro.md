# deeptrace：会自己看病的监控 Agent

> **TL;DR**：deeptrace 是一个轻量级监控 Agent，27 个插件覆盖服务器核心风险点，单二进制零依赖。与众不同的是——告警触发后，它会自动调用 70+ 诊断工具分析根因，把诊断报告和告警一起推给你。登录机器后还可以用 `deeptrace chat` 和 AI 对话排障，不用记命令。
...
deeptrace（猫爪）是一个带 AI 大脑的轻量监控 Agent。它做两件事：
...
这是最核心的能力。你不需要做任何事——配置好 AI 之后，告警触发时 deeptrace 会自动：
...
deeptrace 不是指标采集器（不与 Prometheus + Node-Exporter 重叠），它只关心**有没有问题**。
...
这部分是 deeptrace 最有价值的地方——**很多问题你不监控就永远不知道：**
...
从 [https://github.com/cprobe/deeptrace/releases](https://github.com/cprobe/deeptrace/releases) 下载对应平台的二进制文件。
...
> **想立刻看到告警？** 打开 `conf.d/p.mem/mem.toml`，把内存告警阈值改小（比如 `warn_ge = 0.01`），运行 `./deeptrace run`，一分钟后就能在终端看到告警事件，再等几个几十秒就可以看到 AI 分析的结论（前提是配置文件里启用了 AI 相关配置）。验证完改回来就好。
...
deeptrace 可能是目前最适合验证 AI 编程能力的开源项目之一。整个项目几乎全部由 AI 编写（框架搭好之后，插件、诊断工具、通知渠道的代码都是 AI 写的），这不是噱头，而是因为项目结构天然对 AI 友好。
...
写完 → `go build` → 改个 toml → `./deeptrace run` → 看终端输出。整个过程不超过几分钟，不需要搭建复杂环境。**这种快速反馈循环对 AI 辅助开发特别友好。**

### 试试这些 prompt

打开你的 AI 编程工具，试试：

> 参考 deeptrace 的 disk 插件实现，帮我写一个新插件 `psi`，检测 Linux 系统的 Pressure Stall Information（读取 `/proc/pressure/cpu`、`/proc/pressure/io`、`/proc/pressure/memory`），当资源压力过大时产出告警事件。

或者更有挑战的：

> 参考 deeptrace 现有的诊断工具，帮我写一个新的诊断工具 `gpu_usage`，读取 NVIDIA GPU 的利用率和显存占用，让 AI 在诊断时能分析 GPU 负载。
...
| eBPF 诊断工具 | 引入更多诊断能力，增强 deeptrace 这名老司机的实力 |
| **你的场景** | 你遇到的"早该监控上的"问题 |
...
- 让 AI 熟悉现在的项目情况
- 跟 AI 探讨 raid 插件的实施必要性，如果违反了 deeptrace 的项目定位就不要加进来
- 跟 AI 探讨插件设计思路
...
deeptrace 的名字取自"猫爪"——轻轻一抓就能抓出问题，然后告诉你为什么。
...
- **用它**：下载试试，5 分钟跑起来
- **和它聊**：`deeptrace chat`，体验用 AI 排障的感觉
- **改它**：用 AI 写个新模块，感受快速反馈的爽感
- **一起做**：贡献代码，让 deeptrace 覆盖更多场景

---

**GitHub**：[https://github.com/cprobe/deeptrace](https://github.com/cprobe/deeptrace)

**微信交流群**：加 `picobyte`，备注 `deeptrace`
