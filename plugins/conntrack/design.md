...
**参考**：Prometheus `node_exporter` 的 conntrack collector。node_exporter 采集指标值，deeptrace 关注异常——当使用率逼近上限时主动告警，而非等到丢包后排查。
...
| `count` | `12345` | 当前连接数 |
| `max` | `262144` | 上限值 |
| `usage_percent` | `4.7%` | 格式化的使用率 |

Ok 事件也携带完整 attrs，便于巡检时确认连接跟踪表容量和当前水位。
...
2. **模块加载后自动激活**（每轮 Gather 重新尝试读文件，无需重启 deeptrace）
...
在容器内读取 `/proc/sys/net/netfilter/nf_conntrack_*` 获取的是**宿主机**的值（默认 namespace 下 proc 不隔离）。若 deeptrace 运行在容器内，监控的是宿主机的连接跟踪表。这通常是期望的行为——连接跟踪表是宿主机级别资源，在容器内监控正好覆盖"宿主机 conntrack 满导致容器网络异常"的场景。
