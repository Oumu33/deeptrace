...
| 属性 | 示例值 | 说明 |
| --- | --- | --- |
| `entries` | `768` | 当前邻居表条目数 |
| `gc_thresh3` | `1024` | 硬上限 |
| `usage_percent` | `75.0%` | 格式化的使用率 |

Ok 事件也携带完整 attrs，便于巡检时确认邻居表容量和当前水位。
...
在容器内读取 `/proc/net/arp` 和 `/proc/sys/net/ipv4/neigh/default/gc_thresh3` 获取的是**宿主机**的值。若 deeptrace 运行在容器内，监控的是宿主机的邻居表。这是期望的行为——邻居表是宿主机级别资源，正好覆盖"宿主机 ARP 表满导致容器网络异常"的场景。

**注意**：Kubernetes 场景下，deeptrace 通常以 DaemonSet 部署在每个节点，刚好一个 deeptrace 实例监控一个节点的邻居表。
