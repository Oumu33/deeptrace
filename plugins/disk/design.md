...
deeptrace 是一个轻量级事件监控系统，采用插件机制。disk 插件参考 Nagios `check_disk` 的思路，用 Go 原生实现，无外部依赖，支持跨平台。
...
- 默认关闭，需要 deeptrace 进程对目录有写权限
- 失败固定产生 Critical 事件
- 临时文件名固定为 `.deeptrace_disk_check`，写入使用 `os.Create`（覆盖写），天然幂等
...
writable_test_file = ".deeptrace_disk_check"  # 相对于挂载点
