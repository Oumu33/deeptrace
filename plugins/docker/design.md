...
| 属性 | 示例值 | 说明 |
| --- | --- | --- |
| `container_id` | `a1b2c3d4e5f6` | 容器短 ID（12 字符） |
| `container_image` | `nginx:1.25` | 镜像名:标签 |
...
| --- | --- | --- |
| `state` | `exited` | 容器状态 |
| `status` | `Exited (137) 5 minutes ago` | Docker 人类可读状态 |

### restart_detected 维度

| 属性 | 示例值 | 说明 |
| --- | --- | --- |
| `restarts_in_window` | `3` | 窗口内重启次数 |
| `window` | `10m` | 检测窗口时长（humanDuration 格式） |
| `restart_count` | `15` | Docker 管理的累计 RestartCount |
| `oom_killed` | `true` | 容器最近一次退出是否被 OOM Kill |
| `exit_code` | `137` | 容器最近一次退出码 |
| `started_at` | `2026-02-28 10:30:00 CST` | 容器启动时间（本地时区） |
| `finished_at` | `2026-02-28 10:29:55 CST` | 上次停止时间（本地时区） |

### health_status 维度

| 属性 | 示例值 | 说明 |
| --- | --- | --- |
| `health_status` | `unhealthy` | 健康检查状态 |
| `health_failing_streak` | `5` | 连续失败次数 |
| `health_last_output` | `connection refused` | 最近一次检查输出（截断至 200 字符） |

### cpu_usage 维度

| 属性 | 示例值 | 说明 |
| --- | --- | --- |
| `cpu_percent` | `85.3%` | CPU 使用率 |
| `cpu_limit` | `2.0 cores` | 分配的 CPU 核数（或 `unlimited`） |
| `online_cpus` | `4` | 宿主机在线 CPU 核数 |
| `cpu_throttled_periods` | `1205` | CFS 限流周期数（仅 Linux cgroup 环境有值） |
| `cpu_throttled_time` | `3.2s` | CFS 限流总时间（仅 Linux cgroup 环境有值） |

CPU throttling 数据来自 stats API 响应中的 `cpu_stats.throttling_data`，**零额外 API 调用成本**。Windows 原生容器无 CFS throttling，对应 attrs 不设置。

### memory_usage 维度

| 属性 | 示例值 | 说明 |
| --- | --- | --- |
| `memory_percent` | `78.5%` | 内存使用率 |
| `memory_used` | `1.2 GiB` | 实际使用量（扣除缓存） |
| `memory_limit` | `2.0 GiB` | 内存限制（或宿主机总内存） |

OK 事件也携带完整 attrs。

### "not found" 容器

当显式名称容器未找到时，无法获取容器数据，事件不携带 `container_id` 和 `container_image` attrs——只有 check 和 target 标签。
...
Gather(q):
    ...
    // 4. 显式名称未匹配 → container_running Critical（无 attrs）
...
| CPU throttling attrs | 有值（CFS cgroup） | 有值（Linux VM 内 cgroup） | 无值（无 CFS），不设置 attrs |
| memory `cache` 字段 | `cache`（cgroup v1）/ `inactive_file`（v2） | 同 Linux | `cache` 可能为 0，直接使用 `usage` |

**关键处理**：
1. **CPU throttling**：`throttling_data.throttled_periods == 0 && throttling_data.throttled_time == 0` 时不设置对应 attrs（Windows 原生容器或未发生限流）
